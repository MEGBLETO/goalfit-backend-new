...
export class WorkoutPlanService {
  ...
  async getWorkoutPlanForUser(userId: string): Promise<any> {
    ...
    try {
      ...
      if (userPlan) {
        ...
        return this.transformWorkoutsForFrontend(
          userPlan.workouts,
          userPlan.date,
        );
      }
  ...
  async generateAndStoreWorkoutPlan(userId: string): Promise<any> {
    ...
    try {
      ...
      const existingDates = await this.getExistingPlanDates(userId, dates);
      const missingDates = dates.filter(
        (date) => !existingDates.includes(date),
      );
      if (missingDates.length === 0) {
        return { message: 'All days already have a workout plan.' };
      }
      const rawPlan = await this.workoutService.generateDailyWorkoutPlans(
        userData,
        missingDates,
      );
  ...
  async generateAndStoreDefaultWorkoutPlan(): Promise<any> {
    ...
    try {
      ...
      const dates = this.generateDefaultDates(7);
      const existingDates = await this.getExistingPlanDates(null, dates);
      const missingDates = dates.filter(
        (date) => !existingDates.includes(date),
      );
      if (missingDates.length === 0) {
        return { message: 'All days already have a default workout plan.' };
      }
      const rawPlan = await this.workoutService.generateDailyWorkoutPlans(
        userData,
        missingDates,
  ...
  async getDefaultWorkoutPlanForUser(userId: string) {
    try {
      ...
      this.logger.error(
        `Error getting default workout plan for user ${userId}:`,
        error.stack,
      );
      throw new InternalServerErrorException(
        'Could not retrieve default workout plan.',
  ...
  // Helper to get existing plan dates for a user or default
  private async getExistingPlanDates(
    userId: string | null,
    dates: string[],
  ): Promise<string[]> {
    const where: any = {
    ...
    const existingPlans = await this.prisma.workoutPlan.findMany({
      where,
      select: { date: true },
    });
